...
/* Definitions for BlueLightTask */
osThreadId_t BlueLightTaskHandle;
const osThreadAttr_t BlueLightTask_attributes = {
  .name = "BlueLightTask",
  .stack_size = 128 * 4,
...
void MY_UART_INIT(void){
	USART1->CR1 |= USART_CR1_UE_Msk; // UART ENABLE
	USART1->CR1 |= USART_CR1_TE_Msk; // Transmitter enable
	USART1->CR1 |= USART_CR1_RE_Msk; // Recieve enable
...
/* Configure the peripherals common clocks */
  ...
  TaskButtonHandle = osThreadNew(StartTask02, NULL, &TaskButton_attributes);

  /* creation of BlueLightTask */
  BlueLightTaskHandle = osThreadNew(StartTask03, NULL, &BlueLightTask_attributes);
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
	  if (Timer == 400)
	  		  Timer = 0;
	  TIM4->CCR2 = 32767.5 + 32767.5 * sin((float)Timer * (2 * M_PI / 400.));
    osDelay(1);
  }
  /* USER CODE END 5 */
}
...
void StartTask02(void *argument)
{
  ...
  {
	  if (blue){
		  xQueueSemaphoreTake(myBinarySem01Handle, 0);
	  	  Buffer = 65535;
...
/**
* @brief Function implementing the BlueLightTask thread.
* @param argument: Not used
* @retval None
*/
